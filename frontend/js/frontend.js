...
// 👀 Use local storage / local file instead of live AJAX
function woosw_get_data() {
    console.log("🛠 woosw_get_data called - using local data");

    // Load from sessionStorage first
    let stored = sessionStorage.getItem('woosw_data_local');
    let response;

    if (stored) {
        response = JSON.parse(stored);
    } else {
        // Or load from a local JSON file if you have one
        try {
            fetch('menu-1.html') // You can change this to 'menu-data.json' if you saved as JSON
                .then(res => res.text())
                .then(html => {
                    // Optionally parse HTML to JSON if needed
                    response = { html_content: html, key: 'local_key' };
                    sessionStorage.setItem('woosw_data_local', JSON.stringify(response));
                    $(document.body).trigger('woosw_data_refreshed', [response]);
                })
                .catch(err => console.error('Failed to load local menu:', err));
            return; // Exit so we don't continue
        } catch (err) {
            console.error('Error fetching local menu:', err);
            return;
        }
    }

    // Use cached / local response
    if (response) {
...
// 👀 Load data without hitting live server
function woosw_load_data() {
    console.log("🛠 woosw_load_data called - using local data");

    if ($storage) {
        try {
            var data = JSON.parse(sessionStorage.getItem('woosw_data_local'));
            ...
            console.log("Local data missing, calling woosw_get_data()");
            woosw_get_data();
        }
    } else {
        woosw_get_data();
